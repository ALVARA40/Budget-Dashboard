...
import { STATIC_FLOW, STATIC_KPI } from '../lib/staticData';
import { fmt$, fmtPct } from '../lib/format';
import { DeltaPill } from '../components/ui/DeltaPill';

// Extended flow data for analytics (last 12 months)
const FULL_FLOW = [
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      <div>
        ...
        <div style={{ fontSize: 12.5, color: 'var(--ink-soft)', marginTop: 3 }}>12-month trend · May 2025 – April 2026</div>
      </div>

      {/* Summary KPIs */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
        {[
          { label: 'Total income (12m)',   value: fmt$(totalIncome),   delta: +6.4,  color: '#2FB37A' },
          { label: 'Total expenses (12m)', value: fmt$(totalExpenses), delta: -2.1,  color: '#D8443F', goodOnUp: false },
          { label: 'Net savings (12m)',    value: fmt$(totalSavings),  delta: +11.8, color: '#7FB3E8' },
          { label: 'Avg savings rate',     value: fmtPct(avgRate, 1),  delta: +3.2,  color: '#1F3F8A' },
        ].map(k => (
          <div key={k.label} className="card" style={{ padding: '16px 18px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              <div style={{ fontSize: 11.5, color: 'var(--ink-soft)', fontWeight: 500 }}>{k.label}</div>
              <DeltaPill value={k.delta} goodOnUp={k.goodOnUp !== false} />
            </div>
          ...
          {TOP_CATEGORIES.map(cat => {
            ...
            return (
              <div key={cat.name} style={{ display: 'grid', gridTemplateColumns: '1fr 100px 200px 60px', alignItems: 'center', gap: 14 }}>
                ...
                <div style={{ fontSize: 13, fontVariantNumeric: 'tabular-nums', fontWeight: 600, color: 'var(--ink)', textAlign: 'right' }}>
                  {fmt$(cat.total)}
                </div>
                ...
                <div style={{ fontSize: 11.5, color: 'var(--ink-muted)', fontVariantNumeric: 'tabular-nums', textAlign: 'right' }}>
                  {pct.toFixed(1)}%
                </div>
              </div>
            );
